= Vision: {project-name}
Ben Eisold (s87191<at>htw-dresden.de); Paula Grahlow (s86261<at>htw-dresden.de); Ahmad Ali Nabizada (s87068<at>htw-dresden.de);  Tuan Khang Pham (s87088<at>htw-dresden.de); Christoph Prasser (s87530<at>htw-dresden.de); Cedrick Richter (s87073<at>htw-dresden.de); Elias Blume (s87142<at>htw-dresdsen.de) 
{localdatetime}
include::../_includes/default-attributes.inc.adoc[]
// Platzhalter für weitere Dokumenten-Attribute


== Einführung
Der Zweck dieses Dokuments ist es, die wesentlichen Bedarfe und Funktionalitäten des Systems {project-system-name} überblicksartig zu beschreiben. Der Fokus liegt auf den Fähigkeiten, die von Stakeholdern und adressierten Nutzern benötigt werden, und der Begründung dieser Bedarfe. Die Details, wie das System {project-system-name} diese Bedarfe erfüllt, werden durch User Stories im Product Backlog sowie dem UX-Konzept beschrieben.

=== Zweck
Der Zweck dieses Dokuments ist es, die wesentlichen Anforderungen an das System aus Sicht und mit den Begriffen der künftigen Anwender zu beschreiben.

=== Gültigkeitsbereich (Scope)
Dieses Visions-Dokument bezieht sich auf das System {project-system-name}, das von T1C entwickelt wird. {project-system-name} ermöglicht es Studierenden, ihre Finanzen effizient und unkompliziert zu verwalten. Das System bietet blitzschnelle Transaktionserfassung, ein intuitives und übersichtliches Dashboard, hohe Sicherheitsstandards sowie Budget- und Kategorienmanagement. Darüber hinaus zielt {project-system-name} darauf ab die finanzielle Situation durch Analysen, Vergleiche und individuelle Sparvorschläge zu optimieren. Beispielsweise kann die Entscheidungsfindung bei finanziellen Engpässen unterstützt werden und durch Planung in der Zukunft vermieden werden. Die Anwendung ist für die Nutzung auf Desktop und Smartphone optimiert, wodurch ein flexibler Zugriff jederzeit möglich ist.

=== Definitionen, Akronyme und Abkürzungen
//Verweis passend zum Belegabgabe-Template se1_belegabgabe_t00.adoc
Siehe <<#glossary, Glossar>>.
//Siehe <<./glossary.adoc#glossary, Glossar>>.

// === Referenzen
// (hier externe Verweise zu anderen Dokumenten, Quellen, Standards etc. einfügen, sofern notwendig)


== Positionierung

=== Fachliche Motivation
//Erläutern Sie kurz den Hintergrund, in dem das Projekt angesiedelt ist. Welches Problem soll gelöst werden, wie ist es entstanden? Welche Verbesserung wird angestrebt. Achten Sie darauf, eine fachliche (organisatorische, betriebswirtschaftliche) Perspektive einzunehmen.

[Ich würde noch auf das grundlegende Problem eingehen -> Warum Finanztracker? Was bringt mir der Tracker an sich?]Derzeit sind Finanztracker entweder umfangreich und unübersichtlich [könnt ihr Beispiele anführen?] oder Nutzer greifen gleich zu fragmentierten Excel-Tabellen. Daher wollen wir eine Lösung entwickeln, die sowohl zeitknappe [was ist ein zeitknapper / datenaffiner Student?] als auch datenaffine Studenten anspricht. Erstere erhalten alle wichtigen Erkenntnisse [welche wichtigen Erkentnisse?], ohne sich tief in das Tool einarbeiten zu müssen. Letztere finden schnell umfangreiche Analysemöglichkeiten, ohne dabei direkt auf funktionale Einschränkungen [welche funktionalen Einschränkungen könnte es geben?] zu stoßen. Unser Grundsatz ist es, dem Nutzer so viele Entscheidungen [gibt es auch hier beispiele für Entscheidungen?] wie möglich abzunehmen, damit die Finanzverwaltung neben dem Studienalltag nicht zur lästigen Pflicht wird, sondern mit minimalem Aufwand wertvolle Impulse für ein nachhaltiges Ausgabeverhalten liefert.



=== Produktvision
//Eine Produktvision beschreibt das Einsatzziel der Anwendung und sorgt für ein gemeinsames Ziel und Verständnis für alle Beteiligten im Projekt. Geben Sie in knapper Form übersichtsartig die Positionierung der angestrebten Lösung im Vergleich zu verfügbaren Alternativen dar. Das folgende Format kann dazu verwendet werden:


[cols="1,3"]
|===
| Für | [Studenten]
| die | [alle Ihre Finanzen auf einen Blick haben wollen]
| ist  | [Fin-App]
| eine  | [Finanzmanagement-Applikation]
| die | [einfach und flexibel auch offline die Finanzverwaltung erleichtert]
| anders als | [momentan verbreitete Finanztracker]
| kann unser Produkt | [kostenlos mit State-of-the-art Features einen einfachen überblick in die Finanzen liefern]
|===


//Beispiel
//[cols="1,3"]
//|===
//| Für | Studierende der HTWD
//| die | zu wenig Zeit in der Mittagspause zum Essen haben
//| ist   | Flying Burger
//| ein  | Lieferdient für Burger
//| der  | per Drohne liefert
//| anders als | landgestützte Lieferdienste
//| kann unser Produkt |  durchs Fenster direkt in den Hörsaal liefern.
//|===

== Stakeholder Beschreibungen

=== Zusammenfassung der Stakeholder

[%header, cols="1,3,3"]
|===
| Name | Beschreibung | Verantwortlichkeiten
| Nutzer*innen (Student*innen)
| Student*innen die die App benutzen wollen. Möchte die eigene finanzielle Situation verbessern/im Auge behalten
| - Wünsche beeinflussen die Hauptfunktionen und Gestaltungsentscheidungen für die Anwendung + 
 - will eine funktionnelle, leicht zu bedienende Anwendung

| Projekt-Team
| Entwickelt das System im Rahmen eines studentischen Softwareprojekts. Ziel ist ein funktionierender, nutzbarer Prototyp
| Setzt fachliche und technische Anforderungen um, organisiert sich selbst, dokumentiert, testet und sorgt für methodisches Arbeiten

| Coach / Dozent:in
| Begleitet das Projekt als Lehrperson oder Modulverantwortliche:r, bewertet das Vorgehen und Ergebnis
| Gibt methodisches Feedback, achtet auf Einhaltung der Projektstruktur, überprüft Dokumentation und Fortschritt

| Review-Team
| Testet das System im Sprint Review, um externe Rückmeldungen zur Nutzbarkeit und Verständlichkeit zu geben
| Bewertet Funktionen aus Nutzersicht, gibt Hinweise zur Benutzerfreundlichkeit und erkennt ggf. Probleme in der Bedienung

| Gesetzgeber
| Gibt rechtliche Rahmenbedingungen z.B. zu Datenschutz, Datenspeicherung oder Nutzungsbedingungen vor
| Stellt Anforderungen an den Umgang mit personenbezogenen Daten, an Transparenz (z.B. AGBs) und an rechtliche Sicherheit


| [Benennung des Stakeholders (als Rolle)]
| [Kurze Beschreibung des Stakeholders und seiner Ziele]
| [Fassen Sie die wesentlichen Verantwortlichkeiten des Stakeholder mit Bezug auf das zu entwickelnde System kurz zusammen, d.h. ihr besonderen Interessen. Beispiele: Dieser Stakeholder sorgt dafür, dass das System gewartet wird / dass die angezeigten Daten aktuell sind / überwacht den Projektfortschritt / usw.]
|===



[%header, cols="1,3,3"]
|===
| Name | Beschreibung | Verantwortlichkeiten

| *Magdalena Meier*
24 J., Kunst-Master
a| * Kreativ, liebt Ästhetik
* Extrovertiert (7/10), innovationsoffen (8/10)
* Kein eigener Computer
* Lebt in Dresden
* Nutzt hauptsächlich iPhone & iPad (5 h/Tag Handy, 3 h/Tag Tablet „Lernen")
a| * Superschnelle Eingabe - Quittung abfotografieren, fertig
* Schöne, minimalistische Visualisierung durch Diagramme statt Tabellen
* Immer „alles auf einen Blick" (Saldo, Budget-Status)
* Hätte gerne einen Vergleich zu den Ausgaben anderer und "Sparvorschläge" durch die App
* Möchte keine Wartezeiten - App muss sofort reagieren, offline-fähig sein
* Will sich nicht mit Technik befassen ("nicht belastend in der Nutzung")

| *Mario Meier*
20 J., Informatik-Bachelor
a| * Analytisch, daten-affin
* Introvertierter (3/10), innovationsoffen (9/10), risikoscheu (8/10)
* Hat bereits Tools getestet
* Spielt & programmiert gern
* Desktop-Power-User (5 h/Tag PC, 1 h/Tag Handy)
a| * Sequenzielles Erfassen am PC (Tastatur-Shortcut-optimiert)
* Tiefe Analysen (Kategorie-Trends, Prognosen, Data-Export)
* Regelmäßiger Abgleich zwischen unregel­mäßigen Einnahmen & Fixkosten
* Hat Schwierigkeiten mit unregel­mäßigen Einnahmen/Ausgaben - Standard-Budgets passen selten
* Möchte beim Unterschreiten eines Finanzminimums benachrichtigt werden
* War mit UI/UX existierender Tracker unzufrieden
* Erwartet ein sicheres Produkt, welches er seine Daten ohne Sorgen anvertrauen kann

| *Mark Hofmann*
21 J., Informatik-Bachelor
a| * Logisch, kreativ
* Introvertierter (3/10), innovationsoffen (8/10), relativ risikoscheu (7/10)
* Entwickelt seine erste App
* Möchte ein gutes Produkt und eine gute Note erreichen
* Aktiver Internetnutzer (6 h/Tag PC, 2 h/Tag Handy)
a| * Möchte nach Abschluss der Entwicklung die App selber nutzen und seine Daten einfach von anderen Apps importieren können
* Ist hauptsächlich für das UI-Design zuständig
* Kümmert sich nebenbei um eine übersichtliche Dokumentation
* Sucht bugs um die Qualität des Produkts hoch zu halten

| *Martin Schubert*
32 J., Übungsleiter, Dipl. Informatiker
a| * Professionell, locker 
* Neutral (5/10), innovationsoffen (8/10), eher risikoscheu (6/10)
* Hat ein erfolgreiches Nebenbusiness
* Möchte von seinen Studenten etwas innovatives sehen
* Computer- und Smartphone affin (5 h/Tag PC, 1.5 h/Tag Handy)
a| * Ist erster Ansprechpartner für die Studenten
* Kümmert sich um 2 weitere Entwicklergruppen
* Möchte sich passiv in die Entwicklung einbringen mit hilfreichen Tipps
* Liebt Code nach ISO-Standarts und hasst Spaghetti-Code
* Organisiert das Deployment der App

| *Heinrich Kraemer*
55 J., Professor, Doktor der Informatik
a| * Professionell, erfahren 
* Relativ extrovertiert (7/10), innovationsneutral (6/10), relativ risikoscheu (7/10)
* Besitzt 30 Jahre Berufserfahrung
* Hat hohe Ansprüche an seine Studenten
* Benutzt Computer und Smartphone nur noch selten außerhalb der Uni (30 m/Tag Handy, 3 h/Tag PC)
a| * Ist für seine Studenten verantwortlich
* Beantwortet Fragen rund um das Projekt
* Sorgt für die Bereitstellung von Notwendigkeiten (wie z.B. der Datenbank) 
* Wird die App nur zum Testen nutzen, denn er vertraut seine Finanzen nur Excel an
| *Justus Mayer* (Elias) - Privater Nutzer und Student
| Möchte seine persönlichen Finanzen unterwegs schnell, übersichtlich und sicher verwalten, um stets den Überblick über Einnahmen, Ausgaben, Budgets und Kontostände zu behalten.
a|
* Trägt neue Einnahmen / Ausgaben möglichst einfach am Smartphone ein  
* Kategorisiert Transaktionen, um Ausgabenschwerpunkte zu erkennen  
* Erstellt und verwaltet Budgets für verschiedene Kategorien  
* Beobachtet Trend-Visualisierungen (z. B. Monatsverlauf, Cash-Flow)  
* Lässt sich warnen, wenn Budgets überschritten oder Kontostände kritisch werden  
* Legt wiederkehrende Buchungen (Miete, Gehalt …) an  
* Meldet Usability-Probleme oder Funktionswünsche an das Team
| *Josef Lehmann* (Ben) -  Privater Nutzer und Student
| Nutzt derzeit noch keinen Finanztracker und möchte sich erstmals einen allgemeinen Überblick über seine Ausgaben verschaffen. Dabei ist ihm wichtig, ein Tool zu verwenden, mit dem er Analysen durchführen und sein Sparziel festlegen kann.
a| 
* Legt Wert auf ausführliche Analysen mit verschiedenen Parametern
* Möchte nur _sehr_ wichtige Benachrichtigungen erhalten, um Ablenkung zu vermeiden
* Bevorzugt ein modernes, minimalistisches Design statt altmodischer Oberflächen
* Bleibt nur dann motiviert, seine Ziele zu verfolgen, wenn spielerische Elemente (Gamification) integriert sind
// | [Benennung des Stakeholders (als Rolle)]
// | [Kurze Beschreibung des Stakeholders und seiner Ziele]
// | [Fassen Sie die wesentlichen Verantwortlichkeiten des Stakeholder mit Bezug auf das zu entwickelnde System kurz zusammen, d.h. ihr besonderen Interessen. Beispiele: Dieser Stakeholder sorgt dafür, dass das System gewartet wird / dass die angezeigten Daten aktuell sind / überwacht den Projektfortschritt / usw.]
|===

Die Verfeinerung der Stakeholder, die das System benutzen, wird im UX-Konzept mittels Personas vorgenommen.


== Produktüberblick

=== Bedarfe und Hauptfunktionen
//Vermeiden Sie Angaben zum Entwurf. Nennen wesentliche Features (Produktmerkmale) auf allgemeiner Ebene. Fokussieren Sie sich auf die benötigten Fähigkeiten des Systems und warum (nicht wie!) diese realisiert werden sollen. Geben Sie die von den Stakeholdern vorgegebenen Prioritäten an.

[%header, cols="4,1,4,1"]
Hier bitte mehr auf Messbarkeit eingehen. Konkreter aufzeigen was die Funktionen tun und wie ihr abharken könnt dass ihr sie erreicht habt.
|===
| Bedarf | Priorität | Features / Funktionen | Geplantes Release

| Authentication
| Soll 
| Account erstellen/Login
| (MVP)


| Schnelle, barrierefreie Erfassung von Ein- & Ausgaben, ohne Tabellenwissen 
| Muss
a| 
- One-Click-Eingabe 
- Schnell zu erreichender Button
- einfache Kategorisierung
| MVP

| Sofort verständliche Finanzübersicht
| Muss 
a| 
- Minimalistisches, visuell ansprechendes Dashboard (Saldo, Budget, Sparziele) [was ist visiuell ansprechend?]
| MVP

| übersichtliche tabellarische Anzeige
| Muss
a|
- Filter anbieten (Kategorien)
| MVP

| Tiefgehende Analyse- und Exportmöglichkeiten für datenaffine Nutzer 
| Soll 
a| - Filter- & Drill-Down-Ansichten - CSV/Excel-Export 
| xx
| Automatische Kategorisierung & Budgetwarnungen zum „Denken abnehmen“ 
| Soll 
a|
- KI-gestützte Kategorisierung 
- Regel-basierte Benachrichtigungen/Vorhersagen 
| xx
| Cross-Device-Nutzung inkl. Offline-Modus 
| Soll a| - Cloud-Sync & lokaler Cache 
| xx
| Sichere Nutzung trotz sensibler Finanz-daten 
| Kann 
a| 
- Ende-zu-Ende-Verschlüsselung - Zwei-Faktor-Authentifizierung 
| xx
// | Integration externer Banken / Apps (Open-Banking)
// | Kann 
// a| 
// - Schnittstellen für Giro- & Kredit-karten-konten 
// | xx
| Preis-günstige Lösung speziell für Studierende 
| Kann 
a| - Freemium-Modell mit rabattierten Upgrade-Plänen
| xx

| OCR Scannen von Rechnungen und Belegen
| Kann
| Vollautomatische Eintragung von Rechnungen und Belegen 
| xx

|===

=== Zusätzliche Produktanforderungen
//Zutreffendes angeben, nicht zutreffendes streichen oder auskommentieren

// Hinweise:

// . Führen Sie die wesentlichen anzuwendenden Standards, Hardware oder andere Plattformanforderungen, Leistungsanforderungen und Umgebungsanforderungen auf
// . Definieren Sie grob die Qualitätsanforderungen für Leistung, Robustheit, Ausfalltoleranz, Benutzbarkeit und ähnliche Merkmale, die Qualitätsanforderungen an das geplante Produkt beschreiben.
// . Notieren Sie alle Entwurfseinschränkungen, externe Einschränkungen, Annahmen oder andere Abhängigkeiten, die wenn Sie geändert werden, das Visions-Dokument beeinflussen. Ein Beispiel wäre die Annahme, dass ein bestimmtes Betriebssystem für die vom System erforderliche Hardware verfügbar ist. Ist das Betriebssystem nicht verfügbar, muss das Visions-Dokument angepasst werden.
// . Definieren Sie alle Dokumentationsanforderugen, inkl. Benutzerhandbücher, Onlinehilfe, Installations-, Kennzeichnungs- und Auslieferungsanforderungen.
// . Definieren Sie die Priorität für diese zusätzlichen Produktanforderungen. Ergänzen Sie, falls sinnvoll, Angaben zu Stabilität, Nutzen, Aufwand und Risiko für diese Anforderungen.

[%header, cols="4,1,1"]
|===
| Anforderung | Priorität | Geplantes Release

| Plattformen Modern Web (Chrome/Firefox/Safari akt.)
| Hoch
| MVP

| Performance: Reaktionszeit < 2 s bei Kernaktionen
| Hoch
| MVP

| Barrierefreiheit
| Mittel
| MVP

| Offline-Fähigkeit mit automatischer Synchronisierung
| Niedrig
| xx

| Datenschutz & Compliance (Ende-zu-Ende-Verschlüsselung)
| Hoch
| xx

| Robustheit: Kein Datenverlust bei Abbruch, automatische Wiederherstellung
| Mittel
| xx

| OCR-Genauigkeit ≥ 90 % bei Standardbelegen
| Niedrig
| xx


| Dokumentation: In-App-Hilfe,  Onboarding-Tour
| Mittel
| xx

|===

