...
| Rücklage | Zurückgelegtes Geld für zukünftige Ausgaben oder Notfälle | Reserve
| Fixkosten | Regelmäßig anfallende, meist gleichbleibende Ausgaben | Laufende Kosten
| Variable Kosten | Unregelmäßige oder in der Höhe schwankende Ausgaben | Flexible Ausgaben
| Einnahme | Geldzufluss z.B. durch Gehalt | Zahlungseingang
| Auswertung | Analyse der Finanzen über Zeiträume hinweg | Report, Bericht
| Buchungszyklus | Wiederkehrendes Zeitintervall für Transaktionen (z.B. monatlich) | (keine)
| Lastschrift | Automatische Abbuchung vom Konto durch Dritte | SEPA-Lastschrift
...
| API | Application Programming Interface | Schnittstelle, über die Programme miteinander kommunizieren und Daten austauschen können
| UI | User Interface | Benutzeroberfläche der Anwendung
| UX | User Experience | Nutzererlebnis bei der Bedienung der Anwendung
| JWT | JSON Web Token | Standard für die sichere Übertragung von Authentifizierungsinformationen
| MVP | Minimum Viable Product | Minimale funktionsfähige Version eine Produkts
| IBAN | International Bank Account Number | Internationale Kontonummer zur eindeutigen Identifikation eines Bankkontos
| TAN | Transaktionsnummer | Einmaliger Code zur Freigabe einer Transaktion
