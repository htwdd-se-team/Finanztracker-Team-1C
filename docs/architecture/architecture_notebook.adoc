...
NestJS Backend mit modularer DDD-Architektur (Domain-Driven Design):
...
* FilterController: Filterverwaltung für gespeicherte Suchabfragen
...
* `UserController` (GET /user/me, GET /user/balance, PATCH /user/emergency-reserve)
* `CategoryController` (CRUD für Kategorien mit Pagination)
* `EntryController` (CRUD für Transaktionen mit Filterung)
* `FilterController` (CRUD für gespeicherte Filter)
...
* `User` (Benutzerdaten, E-Mail, Passwort-Hash, Namen, Notgroschen)
* `Category` (Kategorien mit Farben, Icons, Benutzer-Zuordnung)
* `Transaction` (Transaktionen mit Typ INCOME/EXPENSE, Beträge in Cent)
* `Filter` (Gespeicherte Filtereinstellungen für Transaktionsabfragen)
* Unterstützung für wiederkehrende Transaktionen mit verschiedenen Intervallen (DAILY, WEEKLY, MONTHLY)
...
* PostgreSQL-Instanz
* PgAdmin für Datenbankadministration (optional)
