...
import { generateRandomEmail, generateRandomPassword, generateRandomName } from "./helpers/auth-helper";
...
describe("DemoController (e2e)", () => {
  ...
  describe("POST /demo/create-account", () => {
    it("should create a demo account with populated data", async () => {
      ...
      // Check categories
      const categories = await api.categories.categoryControllerList({ take: 30 });
      ...
      const recurring = await api.entries.entryControllerGetScheduledEntries({ take: 30 });
